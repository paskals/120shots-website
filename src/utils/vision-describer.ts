...
export class GoogleVisionImageDescriber {
  ...
  async describeImage(
    ...
    try {
      ...
      const labels =
        visionData.labelAnnotations?.map((l) => l.description) || [];
      const landmarks =
        visionData.landmarkAnnotations?.map((l) => l.description) || [];
  ...
  async describeImageBatch(
    ...
    console.log(
      `🔄 Processing ${imageUrls.length} images with ${delayMs}ms delay between requests...`,
    );
  ...
  private getFeaturesByDetailLevel(detailLevel: string) {
    if (detailLevel === "basic") {
      return [{ type: "LABEL_DETECTION", maxResults: 5 }];
    }
  ...
  private formatDescription(response: any, detailLevel: string): string {
    ...
    if (response.labelAnnotations?.length > 0) {
      const photographyRelevantLabels = this.filterPhotographyLabels(
  ...
  private filterPhotographyLabels(labelAnnotations: any[]): any[] {
    ...
    return labelAnnotations.filter(
      (label) =>
        !skipLabels.some((skip) =>
          label.description.toLowerCase().includes(skip.toLowerCase()),
        ),
    );
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const argv = yargs(hideBin(process.argv))
    .command("describe <urls..>", "Generate descriptions for image URLs", {
      detailLevel: {
        alias: "d",
  ...
  if (argv._.includes("describe") && argv.urls && Array.isArray(argv.urls)) {
    ...
    console.log(
      'Example: tsx vision-describer.ts describe "https://cdn.120shots.com/images/roll1/photo1.webp"',
    );
  }
}
